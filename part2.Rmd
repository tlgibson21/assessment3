...
```{r mhp k-mers}
prots_mhp <- unlist(prot_mhp)

mycounts_mhp <- count(prots_mhp,wordsize=3,alphabet=aa_m)
...
```{r mhp k-mers under}
myfreq_mhp <- count(prots_mhp,wordsize=3,alphabet=aa_m,freq=TRUE)
...
mycounts_ecoli <- count(prots_ecoli,wordsize=3,alphabet=aa_e)
...
myfreq_ecoli <- count(prots_ecoli,wordsize=3,alphabet=aa_e,freq=TRUE)
...
under_ecoli <- head(sorted_ecoli, n = 10L)
